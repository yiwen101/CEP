...
Plots correctness metrics and token usage (including reasoning tokens) per model per domain.
"""
...
def get_metric_value(metrics: Dict[str, float], metric_name: str) -> float:
    ...
    if metric_name == 'correct':
        ...
        return metrics.get('avg_tokens_used', metrics.get('tokens_used', 0.0))
    elif metric_name == 'reasoning_tokens':
        return metrics.get('avg_reasoning_tokens', metrics.get('reasoning_tokens', 0.0))
    elif metric_name == 'execution_time':
        return metrics.get('avg_execution_time', metrics.get('execution_time', 0.0))
...
def create_bar_plot(data: Dict[str, Dict], domain: str, model: str, metric: str, output_path: str):
    ...
    for method, method_data in model_data.items():
        if 'metrics' in method_data:
            ...
            if metric == 'tokens_used':
                output_tokens.append(get_metric_value(method_data['metrics'], 'tokens_used'))
                reasoning_tokens.append(get_metric_value(method_data['metrics'], 'reasoning_tokens'))
            else:
                output_tokens.append(get_metric_value(method_data['metrics'], metric))
    
    if not methods:
        logging.warning(f"No valid methods found for {domain} - {model}")
        return
    
    # For token plots, sort by output tokens
    if metric == 'tokens_used':
        sorted_indices = np.argsort(output_tokens)[::-1]  # Sort in descending order
        methods = [methods[i] for i in sorted_indices]
        output_tokens = [output_tokens[i] for i in sorted_indices]
        reasoning_tokens = [reasoning_tokens[i] for i in sorted_indices]
    else:
        sorted_indices = np.argsort(output_tokens)[::-1]  # Sort in descending order
        methods = [methods[i] for i in sorted_indices]
        output_tokens = [output_tokens[i] for i in sorted_indices]
    ...
    if metric == 'tokens_used':
        # Calculate non-reasoning tokens
        non_reasoning_tokens = [out - reason for out, reason in zip(output_tokens, reasoning_tokens)]
        
        # Create stacked bar plot for tokens
        bars1 = plt.bar(range(len(methods)), non_reasoning_tokens, 
                       label='Non-reasoning Output', color='#2ecc71', alpha=0.8)
        bars2 = plt.bar(range(len(methods)), reasoning_tokens,
                       bottom=non_reasoning_tokens, label='Reasoning Output', color='#3498db', alpha=0.8)
        
        # Add value labels on bars
        for i in range(len(methods)):
            total = output_tokens[i]
            non_reason = non_reasoning_tokens[i]
            reason = reasoning_tokens[i]
            
            # Total tokens at the top
            plt.text(i, total + max(output_tokens) * 0.01,
                    f'Total: {int(total):,}', ha='center', va='bottom', fontsize=8)
            
            # Show reasoning tokens in the middle if significant enough
            if reason > total * 0.1:  # Only show if reasoning is >10% of total
                plt.text(i, non_reason + reason/2,
                        f'Reasoning: {int(reason):,}', ha='center', va='center',
                        fontsize=8, color='white')
        plt.legend()
    else:
        bars = plt.bar(range(len(methods)), output_tokens, color='#2ecc71', alpha=0.8)
        # Add value labels on top of bars
        for i, v in enumerate(output_tokens):
            plt.text(i, v + 0.01, f'{v:.3f}', ha='center', va='bottom', fontsize=8)
    
    # Customize plot
    metric_name = 'Correctness' if metric == 'correct' else 'Token Usage'
    plt.title(f'{metric_name} for {domain} - {model}', fontsize=14, pad=20)
    plt.ylabel(f'{metric_name} {"Score" if metric == "correct" else "Count"}', fontsize=12)
...
def create_summary_table(data: Dict[str, Dict], domain: str, model: str, output_path: str):
    ...
    for method, method_data in model_data.items():
        if 'metrics' in method_data:
            ...
            output_tokens = get_metric_value(metrics, 'tokens_used')
            reasoning_tokens = get_metric_value(metrics, 'reasoning_tokens')
            table_data.append({
                'Method': method,
                'Correctness': get_metric_value(metrics, 'correct'),
                'Total Output': output_tokens,
                'Reasoning Part': reasoning_tokens,
                'Non-reasoning Part': output_tokens - reasoning_tokens,
                'Time (s)': get_metric_value(metrics, 'execution_time')
            })
    ...
    # Sort by total output tokens
    table_data.sort(key=lambda x: x['Total Output'], reverse=True)
    
    # Create figure and table
    fig, ax = plt.subplots(figsize=(18, len(table_data) * 0.4 + 1))  # Increased width for new column
    ax.axis('tight')
    ax.axis('off')
    ...
    cell_text = [[
        d['Method'],
        f"{d['Correctness']:.3f}",
        f"{int(d['Total Output']):,}",
        f"{int(d['Reasoning Part']):,}",
        f"{int(d['Non-reasoning Part']):,}",
        f"{d['Time (s)']:.2f}"
    ] for d in table_data]
    
    table = ax.table(cellText=cell_text,
                    colLabels=['Method', 'Correctness', 'Total Output', 'Reasoning Part', 'Non-reasoning Part', 'Time (s)'],
                    cellLoc='center',
                    loc='center')
        ...
        try:
            ...
            for domain in data:
                ...
                for model in data[domain]:
                    ...
                    tokens_plot_path = os.path.join(domain_dir, f'{domain}_{model}_tokens.png')
                    create_bar_plot(data, domain, model, 'tokens_used', tokens_plot_path)
